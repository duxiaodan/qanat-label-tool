  ...
  },
  {
    ...
    blocks: [
      { t: 'ul', items: [
        ...
        '<b>Layer toggles</b> (top bar): the model\'s probability <b>Heatmap</b>, <b>Original labels</b> (original shaft points, including saved corrections), <b>User labels</b> (everyone\'s labels), <b>Accurate TIFs</b> (green = cells whose imagery was verified well-aligned), and <b>Filter mask</b> (see below).',
  ...
  },
  {
    ...
    blocks: [
      ...
      { t: 'ul', items: [
        ...
        '<b>Move</b>: select one or more labels, then drag any selected label. Multiple labels move together, keeping their relative positions and stopping together at the crop boundary. With a single polyline selected, grabbing a vertex moves that vertex; grabbing a segment moves the whole line. <b>Esc during a drag</b> restores the entire selection. Release the mouse to keep the move, then Save work. Use Undo last or Ctrl/Cmd+Z to undo the move before saving.',
        '<b>Undo last</b> (or <b>Ctrl/Cmd+Z</b>) reverses the most recent edit: adding points or polyline vertices, finishing/cancelling a polyline, moving labels, deleting labels, or Clear this crop. A group move or batch deletion is one undo step. Up to 100 steps are kept for the current crop. Saving (including a failed save attempt), refreshing, or leaving the crop clears this history; pending edits are still kept for save/retry. Undoing a finished polyline returns it to drawing mode. Undoing original-label edits requires Original labels and Edit originals. Undoing edits to other people\'s labels requires Edit others. <b>Clear this crop</b> removes all of your own labels and can be undone before saving.',
  ...
  },
  {
    ...
    blocks: [
      ...
      { t: 'ul', items: [
        ...
        '<b>Edit originals</b> (crop toolbar, superusers only): turn on Original labels and Edit originals to move or delete the red shaft points. You can select an original point and read History without enabling editing. Box selection and group movement also work with user labels. User labels may be hidden while editing originals. Use Save work to share changes; Undo last reverses unsaved changes. The source GeoJSON remains unchanged.',
        '<b>Show deleted originals</b>: show deleted original points as dashed red circles with a cross on the image. Select the point you care about, then click the same <b>History…</b> button to inspect it or restore an earlier state. Deleted points can be selected for history; dragging or pressing Delete will not change them.',
        '<b>Snapshots</b> (top-bar button): save the complete current state of user labels as one snapshot, with an optional note. Each snapshot is identified by a short hash (like a git commit). A dim <b>≡ hash</b> badge means that snapshot\'s content is identical to an earlier one. <b>Restore…</b> returns the whole project to a snapshot\'s state — a safety snapshot of the current state is taken automatically first.',
        '<b>History…</b> (crop toolbar): select exactly one label on the image, then open its history. Original labels and saved user labels share this panel; a mixed or multiple selection disables it. Original history includes its imported position and saved moves, deletions and restorations. <b>Restore this state</b> on an original creates an undoable local edit; close History and use Save work to share it. For user labels, restoration writes the selected earlier version immediately, as before. Original corrections remain separate from user-label snapshots.',
      ] },
    ...
    blocks: [
      { t: 'table',
        ...
        rows: [
          ...
          ['Crop view', 'press + drag a selected mark', 'move the selection together'],
